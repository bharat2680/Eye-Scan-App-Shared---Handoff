...
def import_pandas():
    try:
        ...
        subprocess.run(
            [sys.executable, "-m", "pip", "install", "-q", "pandas", "openpyxl", "xlrd"],
            check=True,
        )
        import pandas as pd
    return pd
...
def _drive_split_zip_candidates(datasets_root: Path) -> dict[str, list[Path]]:
    return {
        "train": [
            datasets_root / "Train.zip",
            datasets_root / "train.zip",
            datasets_root / "Chaksu_Train.zip",
        ],
        "test": [
            datasets_root / "Test.zip",
            datasets_root / "test.zip",
            datasets_root / "Chaksu_Test.zip",
        ],
    }


def _find_existing_raw_zips(datasets_root: Path) -> list[Path] | None:
    for candidate in _drive_zip_candidates(datasets_root):
        if candidate.exists():
            return [candidate]

    split_candidates = _drive_split_zip_candidates(datasets_root)
    found: list[Path] = []
...
def _download_latest_figshare_chaksu() -> tuple[list[Path], dict]:
    ...
    if not zip_files:
        raise RuntimeError(f"No zip files found in Figshare article {FIGSHARE_ARTICLE_ID}")

    preferred: list[dict] = []
    for wanted in ("train.zip", "test.zip"):
        for file_info in zip_files:
            if str(file_info.get("name", "")).lower() == wanted:
                preferred.append(file_info)
                break
    chosen_files = preferred if preferred else zip_files
    ...
    for file_info in chosen_files:
        ...
        if not download_url:
            raise RuntimeError(f"Figshare file metadata did not include download_url: {file_info}")
        file_name = str(file_info.get("name", "Chaksu.zip"))
        target_path = FIGSHARE_FALLBACK_DIR / file_name
    ...
    return target_paths, source_info
...
def _extract_image_keys_from_cell(value: object) -> list[str]:
    text = str(value).strip()
    if not text:
        return []

    matches = re.findall(r"[^\\s,;|]+?\\.(?:jpg|jpeg|png|bmp)", text, flags=re.IGNORECASE)
    if matches:
        keys = [_normalize_image_key(match) for match in matches]
...
def _extract_decisions_from_tables(decision_root: Path) -> tuple[dict[str, int], dict]:
    ...
    for table_path in table_paths:
        ...
        for row in frame.itertuples(index=False, name=None):
            ...
            keys = _extract_image_keys_from_cell(row_dict.get(image_col))
            if not keys:
                continue
            parsed_rows += 1
...
def main() -> None:
    ...
    raw_zips = _find_existing_raw_zips(datasets_root)
    source_info = None
    if raw_zips is None:
        raw_zips, source_info = _download_latest_figshare_chaksu()
    else:
        source_info = {
            ...
            "file_names": [path.name for path in raw_zips],
            "download_url": None,
        }

    print("Using raw archives:", [str(path) for path in raw_zips])
    counts, debug = _extract_chaksu_dataset(raw_zips, raw_extract_root, dataset_root)
    ...
    metrics = {
        ...
        "dataset_source": {
            "raw_archive_zips": [str(path) for path in raw_zips],
            **source_info,
        },
        "dataset_debug": debug,
